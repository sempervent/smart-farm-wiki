...
[Edit this page](https://github.com/longhorn/website/edit/master/content/docs/1.11.1/deploy/install/_index.md)
...
> **Note**: This quick installation guide uses some configurations which are not for production usage. Please see [Best Practices](https://longhorn.io/docs/1.11.1/best-practices/) for how to configure Longhorn for production usage.

Longhorn can be installed on a Kubernetes cluster in several ways:

- [Rancher Catalog App](https://longhorn.io/docs/1.11.1/deploy/install/install-with-rancher)
- [kubectl](https://longhorn.io/docs/1.11.1/deploy/install/install-with-kubectl/)
- [Helm](https://longhorn.io/docs/1.11.1/deploy/install/install-with-helm/)
- [Helm Controller](https://longhorn.io/docs/1.11.1/deploy/install/install-with-helm-controller/)
- [Fleet](https://longhorn.io/docs/1.11.1/deploy/install/install-with-fleet/)
- [Flux](https://longhorn.io/docs/1.11.1/deploy/install/install-with-flux/)
- [ArgoCD](https://longhorn.io/docs/1.11.1/deploy/install/install-with-argocd/)

To install Longhorn in an air gapped environment, refer to [Air Gap Installation](https://longhorn.io/docs/1.11.1/deploy/install/airgap).

For information on customizing Longhorn’s default settings, refer to [Customizing Default Settings](https://longhorn.io/docs/1.11.1/advanced-resources/deploy/customizing-default-settings).

For information on deploying Longhorn on specific nodes and rejecting general workloads for those nodes, refer to the section on [Taints and Tolerations](https://longhorn.io/docs/1.11.1/advanced-resources/deploy/taint-toleration).
...
[Longhorn Command Line Tool](https://longhorn.io/docs/1.11.1/advanced-resources/longhornctl/) can be used to check the Longhorn environment for potential issues.

For the minimum recommended hardware, refer to the [best practices guide](https://longhorn.io/docs/1.11.1/best-practices/#minimum-recommended-hardware).
...
- Google Kubernetes Engine (GKE): See [Longhorn CSI on GKE](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/csi-on-gke).
- K3s clusters: See [Longhorn CSI on K3s](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/csi-on-k3s).
- RKE clusters with CoreOS: See [Longhorn CSI on RKE and CoreOS](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/csi-on-rke-and-coreos).
- OCP/OKD clusters: See [OKD Support](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/okd-support).
- Talos Linux clusters: See [Talos Linux Support](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/talos-linux-support).
- Container-Optimized OS: See [Container-Optimized OS Support](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/container-optimized-os-support).
...
- Longhorn CSI Plugin
	- /: For host checks via the NFS customer mounter (deprecated). Note that, this will be removed in the future release.
		...
		- /var/lib/kubelet/plugins\_registry: The path where the node-driver-registrar registers the CSI plugin with kubelet.
...
- RHEL, CentOS, and EKS *(EKS Kubernetes Worker AMI with AmazonLinux2 image)*: Run the following commands:
	...
	systemctl start iscsid
	```
- Talos Linux: See [Talos Linux Support](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/talos-linux-support).
- Container-Optimized OS: See [Container-Optimized OS Support](https://longhorn.io/docs/1.11.1/advanced-resources/os-distro-specific/container-optimized-os-support)

Please ensure iscsi\_tcp module has been loaded before iscsid service starts. Generally, it should be automatically loaded along with the package installation.
...
We also provide an `iscsi` installer to make it easier for users to install `open-iscsi` automatically. You can use the [Longhorn CLI](https://longhorn.io/docs/1.11.1/advanced-resources/longhornctl/) to install the prerequisites.
...
In rare cases, it may be required to modify the installed SELinux policy to get Longhorn working. If you are running an up-to-date version of a Fedora downstream distribution (e.g. Fedora, RHEL, Rocky, CentOS, etc.) and plan to leave SELinux enabled, see [the KB](https://longhorn.io/kb/troubleshooting-volume-attachment-fails-due-to-selinux-denials) for details.
...
We also provide an `nfs` installer to make it easier for users to install `nfs-client` automatically. You can use this [Longhorn CLI](https://longhorn.io/docs/1.11.1/advanced-resources/longhornctl/) to install the prerequisite.
...
The `longhornctl` tool is a CLI for Longhorn operations. For more information, see [Command Line Tool (longhornctl)](https://longhorn.io/docs/1.11.1/advanced-resources/longhornctl/).
...
```shell
longhornctl --kubeconfig ~/.kube/config --image longhornio/longhorn-cli:v1.11.1 install preflight
```
