...
[Edit this page](https://github.com/longhorn/website/edit/master/content/docs/1.11.1/advanced-resources/os-distro-specific/csi-on-k3s.md)
...
If you want to deploy these new Longhorn versions on versions before k3s v0.10.0, you need to set `--kubelet-root-dir` to `<data-dir>/agent/kubelet` for the Deployment `longhorn-driver-deployer` in `longhorn/deploy/longhorn.yaml`. `data-dir` is a `k3s` arg and it can be set when you launch a k3s server. By default it is `/var/lib/rancher/k3s`.
...
You can override the root-dir detection by setting environment variable `KUBELET_ROOT_DIR` in [https://github.com/longhorn/longhorn/blob/v1.11.1/deploy/longhorn.yaml](https://github.com/longhorn/longhorn/blob/v1.11.1/deploy/longhorn.yaml).
