...
- `--keep-tag` keep all snapshots which have all tags specified by this option (can be specified multiple times). The `forget` command will exit with an error if all snapshots in a snapshot group would be removed as none of them have the specified tags.
...
Since restic 0.17.0, it is possible to delete all snapshots for a specific host, tag or path using the `--unsafe-allow-remove-all` option. The option must always be combined with a snapshot filter (by host, path or tag). For example the command `forget --tag example --unsafe-allow-remove-all` removes all snapshots with tag `example`.
...
- `--repack-small` if set will repack pack files below 80% of target pack size. The default value is false.
- `--repack-smaller-than` will repack all packfiles below the size of `--repack-smaller-than`. This allows repacking packfiles that initially came from a repository with a smaller `--pack-size` to be compacted into larger packfiles.
