<!-- Source: inbox drop 2026-04-18; original filename: Logs.md -->
...
Logs can be marked as "pending", "done", or "abandoned". Pending Logs represent planned events, while done Logs represent events that actually happened. If an event is never completed and there is no intention of completing it, the Log can be marked as "abandoned" to make that clear while preserving it for future reference.

Every change that is made to a Log is stored as a revision, so that it's possible to see how a plan evolves over time until it eventually becomes a canonical record of the event that took place.
...
See the "Geometry" attribute of [Assets](https://farmos.org/model/type/asset#geometry). For more information, see [farmOS Location Logic](https://farmos.org/model/logic/location).
...
For more information, see [farmOS Location Logic](https://farmos.org/model/logic/location).
...
For more information, see [farmOS Group Membership Logic](https://farmos.org/model/logic/group).
...
For example: if you are applying an input to a pasture (represented as an Asset of type "Land"), then the action is happening both *TO* the pasture and *IN* the pasture. In this case, the pasture Asset can be referenced in either or both the "Asset" and "Location" fields. This decision is left up to the [convention](https://farmos.org/model/convention) of the end-user or module that implements it.

When combined with the "Is movement" attribute (above), this will move all Assets referenced on the Log to the locations specified. For more information, see [farmOS Location Logic](https://farmos.org/model/logic/location).

See also: the "Is location" attribute of [Assets](https://farmos.org/model/type/asset#is_location).

#### Quantities

Quantities are records that contain quantitative data. These are used to represent things like input amounts, harvest totals, time tracking, etc. For more information, see: [Quantities](https://farmos.org/model/type/quantity).
...
For more information, see [farmOS Membership Logic](https://farmos.org/model/logic/group).
