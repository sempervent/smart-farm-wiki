...
## [2026-04-21] policy | AGENTS — PDF `source_ids` require body links to `*-extracted.md` (CI)

- **`AGENTS.md`**: Ingest workflow now states explicitly that any `source_ids` entry ending in `.pdf` requires a **markdown** link in the **body** to `{stem}-extracted.md` (CI via `pytest tests/test_validate_raw_pdf_links.py` / `scripts/validate_raw_pdf_links.py`); updated agent checklist, definition-of-done ingest row, and scripts table for `validate_raw_pdf_links.py`.
- **Context**: avoids deploy failures like missing extract links on [`tennessee-farm-business-compliance-official-layer-cluster-2026-04-22.md`](source-notes/tennessee-farm-business-compliance-official-layer-cluster-2026-04-22.md).
