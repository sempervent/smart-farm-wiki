---
title: East Tennessee two-site farm business plan — executive summary
page_type: analysis
status: active
created: 2026-04-17
updated: 2026-04-17
review_status: unreviewed
tags:
  - business-plan
  - executive-summary
---

# Executive summary

**Hub**: [`Planning framework`](east-tennessee-two-site-farm-business-plan-framework.md)

## What this is

A **wiki-native**, **phased** business plan for an **East Tennessee** **two-site** operation: **~5 acres** = **home + control center** (smart house, workshop/lab, telemetry spine); **~120 acres** = **primary production**, **~35 minutes** away. **Objective order**: **profit** → **low labor** → **resilience**. **Horizon**: **10 years** from side income toward **possible** **salary replacement**—**not** **guaranteed**.

## Strategic choice (strategy family)

**Phased mixed** **enterprise** with **grazing-led** **beef** **as** **primary** **120-acre** **engine**, **optional** **lease/contract** **stabilizers**, and **deferred** **high-labor** **or** **long-payback** **enterprises** **until** **validated**. **Rationale** **and** **branching** **logic**: [`Recommended enterprise strategy`](east-tennessee-two-site-farm-business-plan-recommended-enterprise-strategy.md). **Full** **scenario** **matrix**: [`Enterprise options analysis`](east-tennessee-two-site-farm-business-plan-enterprise-options-analysis.md).

## Operating model (one paragraph)

**Batch** **field** **work** **at** **120** **around** **a** **weekly** **rhythm**; **use** **homestead** **for** **planning**, **records**, **broker**, **spares**, **and** **development**. **Telemetry** **targets** **water**, **power**, **and** **gates** **first** **to** **reduce** **panic** **trips**—**not** **cameras** **everywhere** **on** **day** **one**.

## Phases (snapshot)

| Phase | Essence |
|-------|---------|
| **0** | **No-regret** **groundwork**, **stop** **rules**, **access** **clarity** |
| **1** | **Land** **intelligence**, **market** **probe**, **pilot** **sensor** |
| **2** | **First** **real** **grazing** **business** **+** **records** |
| **3** | **Scale** **or** **second** **stream** **under** **labor** **cap** |
| **4** | **Salary-replacement** **architecture** **if** **metrics** **hold** |

**Detail**: [`10-year roadmap`](east-tennessee-two-site-farm-business-plan-10-year-roadmap.md).

## Economics (qualitative)

- **Short-term**: **supplemental** **income** **(lease**, **small** **sales** **)** **while** **off-farm** **cash** **dominates**.
- **Long-term**: **operating** **margin** **+** **disciplined** **scale** **toward** **household** **cash** **target** **(fill** **number** **in** **revenue** **page** **)**.

## Top risks (see full register)

1. **Price** **/** **weather** **margin** **squeeze**  
2. **Debt** **vs** **thin** **margin**  
3. **Labor** **surge** **without** **coverage**  
4. **Telemetry** **false** **confidence** **/** **alert** **fatigue**  
5. **Liability** **/** **zoning** **surprises**  

## What must be true before heavy CAPEX

**G1** **gate**: **land** **+** **water** **+** **legal** **use** **clarified**; **no** **full** **herd** **before** **[`validation backlog`](east-tennessee-two-site-farm-business-plan-validation-backlog.md)** **minimums**.

## Document map

| Topic | Page |
|-------|------|
| Vision, objectives, stop rules | [`Vision`](east-tennessee-two-site-farm-business-plan-vision-and-constraints.md) |
| Two-site model + control center | [`Two-site operating context (ET)`](east-tennessee-two-site-farm-business-plan-two-site-operating-context.md); ops model [`5 ac / 120 ac`](two-site-operations-model-5ac-homebase-120ac-production.md) |
| Strategy family + rationale | [`Recommended enterprise strategy — phased East Tennessee path`](east-tennessee-two-site-farm-business-plan-recommended-enterprise-strategy.md) |
| Smart tech + automation discipline | [`Smart technology and telemetry strategy — control center on 5 acres`](east-tennessee-two-site-farm-business-plan-smart-tech-strategy.md) |
| Labor + weekly rhythm | [`Labor model and weekly operating rhythm`](east-tennessee-two-site-farm-business-plan-labor-and-family-model.md) |
| Capital + infrastructure sequence | [`Capital plan and infrastructure sequencing`](east-tennessee-two-site-farm-business-plan-capital-and-financing.md) |
| Revenue + milestones | [`Revenue model and milestones`](east-tennessee-two-site-farm-business-plan-revenue-and-phased-income.md) |
| Risks | [`Risk register and mitigation strategy`](east-tennessee-two-site-farm-business-plan-risk-register.md) |
| 10-year phases | [`Roadmap`](east-tennessee-two-site-farm-business-plan-10-year-roadmap.md) |
| Validation tasks | [`Validation backlog and decision gates`](east-tennessee-two-site-farm-business-plan-validation-backlog.md) |

### Operational artifacts (fillable; execution layer)

| Artifact | Page |
|----------|------|
| Two-site ops — 5 ac home / 120 ac production | [`Two-site operating model — 5-acre home base and 120-acre farm`](two-site-operations-model-5ac-homebase-120ac-production.md) |
| Family labor + coverage matrix | [`Family labor and coverage matrix`](family-labor-model-and-coverage-matrix-two-site-smart-farm.md) |
| Instrumentation priority matrix | [`Instrumentation priority matrix`](instrumentation-priority-matrix-two-site-smart-farm.md) |
| Capital phasing 0–10 years | [`Capital phasing table`](capital-phasing-table-years-0-10-two-site-smart-farm.md) |
| Revenue milestone ladder | [`Revenue milestone model`](revenue-milestone-model-supplemental-to-salary-replacement.md) |
| Business risk register (review matrix) | [`Business risk register — two-site smart farm`](business-risk-register-two-site-smart-farm.md) |
| Manual fallback + degraded modes | [`Manual fallback — critical operations`](manual-fallback-degraded-modes-critical-operations.md) |
| Validation before major spend | [`Validation backlog — before major spend`](validation-backlog-before-major-spend-two-site-smart-farm.md) |

---

## Known facts

| ID | Fact |
|----|------|
| K1 | **Structure** **and** **intent** **as** **stated** **in** **hub** |

## Assumptions

| ID | Assumption |
|----|------------|
| A1 | **Readers** **will** **fill** **numeric** **targets** **locally** |

## Open questions

| ID | Question |
|----|----------|
| Q1 | **Household** **salary** **replacement** **$** **?** |
| Q2 | **Go** **/** **no-go** **date** **for** **Phase** **2** **?** |

## Links

- [`Business viability and farm economics — gap analysis`](business-viability-and-farm-economics-gap-analysis.md)
- [`Strategic audit — decision-safe operations`](strategic-audit-decision-safe-operations.md)
