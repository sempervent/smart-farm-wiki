---
title: "Source note: Animals journal PDF (animals-14-01645)"
page_type: source_note
status: active
created: 2026-04-17
updated: 2026-04-19
source_ids:
  - raw/processed/2026/animals-14-01645.pdf
tags:
  - pdf
  - livestock
  - research
review_status: unreviewed
---

# Source note: Animals journal PDF (animals-14-01645)

**Raw PDF**: [`raw/processed/2026/animals-14-01645.pdf`](../../raw/processed/2026/animals-14-01645.pdf)  
**Machine extract**: [`raw/processed/2026/animals-14-01645-extracted.md`](../../raw/processed/2026/animals-14-01645-extracted.md)

**Summary**

Peer-reviewed **PDF** import; filename matches common **MDPI *Animals*** article numbering (confirm exact title, authors, and DOI inside the PDF or via the publisher). Use for livestock/animal-science citations once bibliographic metadata is extracted from the document.

**Downstream pages**

- [`Backyard livestock and homestead animal sources`](../topics/backyard-livestock-and-homestead-animals.md)
